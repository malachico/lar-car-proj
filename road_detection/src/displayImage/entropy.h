...
void CallBackFunc(int event, int x, int y, int flags, void* param)
{
 ...
 if  ( event == EVENT_LBUTTONDOWN )
 {
   ...
   cout<< (arr[x/ENT_WIN_SIZE][y/ENT_WIN_SIZE].entropy) << endl;
...
if((compareEntropies(arr[t.fx][t.fy].entropy,arr[t.x][t.y].entropy))){
  ...
  edges.push_back(arr[t.x][t.y].serial);
}
else
{
  arr[t.x][t.y].id = id;
  ...
  for(int i=0;i<rs;i++)
   for(int j=0;j<cls;j++)
   {
     ...
     if (arr[i][j].id == ID)
       ...
       clr = 18;
     if (find(edges.begin(), edges.end(), arr[i][j].serial) != edges.end())
      {
        rectangle(image,rec,Scalar(((clr*5)%255),((clr*9)%255),((clr*12)%255)),1,8,0);
      }else{
        clr = (arr[i][j].id) % 255;
        rectangle(image,rec,Scalar(clr),1,8,0);
    }
   }
