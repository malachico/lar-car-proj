  ...
  : jointDeadbandPercent(0.02)
{
  ...
  this->fLwheelSteeringPgain = 2000;
  this->fRwheelSteeringPgain = 2000;
...
//                          this->fnrSwitchForce, -this->fnrSwitchForce);
  printf("Steering PID gains:(%.0f,%.0f),(%.0f,%.0f),(%.0f,%.0f)\n",this->fLwheelSteeringPgain,this->fRwheelSteeringPgain,this->fLwheelSteeringIgain,this->fRwheelSteeringIgain,this->fLwheelSteeringDgain,this->fRwheelSteeringDgain);
...
//   this->fnrSwitchState = this->fnrSwitchJoint->GetAngle(0).Radian();
  ...
  if (dt > 0)
  {
    ...
    steerError = (this->handWheelCmd-this->handWheelState)*(1-this->fLwheelSteeringIgain) + steerError*this->fLwheelSteeringIgain;
    double steerCmd = steerError*this->fLwheelSteeringPgain + (steerError - lastErr) * this->fLwheelSteeringDgain;//  this->handWheelPID.Update(steerError, dt);
    if(steerCmd > 1) steerCmd = 1;
    if(steerCmd < -1) steerCmd = -1;
    //printf("des: %.3f   is: %.3f   Error: %.3f   STEER CMD: %.3f   last: %.3f\n",this->handWheelCmd, this->handWheelState ,steerError,steerCmd,lastErr);
