...
class HeightMap
{
	 ...
	 */
        Mat buildGUI(int rotation, double px, double py, int enlarger=3);
        void displayConsole();
	...
	void setBlendFunc(double (*f)(double lastHeight, double curHeight));
	...
	double (*_blendFunc)(double lastHeight, double curHeight);
};
