...
void HeightMap::setRelativeHeightAt(int x, int y, double height)
{
    ...
    else _at(x,y) = _blendFunc(_at(x,y), height);
}
...
double HeightMap::lowPassFilter(double last, double curr)
{
  return last * 0.95 + curr * 0.05;
}

double HeightMap::maxHeightFilter(double last, double curr)
{
  return last > curr ? last : curr;
}

double HeightMap::overrideFilter(double last, double curr)
{
  return last;
}

void HeightMap::setBlendFunc(double (*f)(double lastHeight, double curHeight))
{
  _blendFunc = f;
}
